...
<div class="bg-white">
	<form action="{{ route('edit_product', $product) }}" method="get">
		@csrf
		<x-secondary-button type="submit">Edit Product</x-secondary-button>
	</form>
	<div class="pt-6">
	    ...
	    <ol role="list" class="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
		 <li>
		   <div class="flex items-center">
			<a href="{{ route('index_product') }}" class="mr-2 text-sm font-medium text-blue-700">&laquo; Back</a>
	  ...
	  <div class="mx-auto mt-6 max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
	    <div class="aspect-h-4 aspect-w-3 hidden overflow-hidden rounded-lg lg:block">
		 <img src="{{ url('storage/' . $product->image) }}" alt="Two each of gray, white, and black shirts laying flat." class="h-full w-full object-cover object-center">
	    </div>
	  ...
	  <div class="mx-auto max-w-2xl px-4 pb-16 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pb-24 lg:pt-16">
	    ...
	    <div class="mt-4 lg:row-span-3 lg:mt-0">
		 ...
		 <p class="text-3xl tracking-tight text-gray-900">Rp. {{ Number::format($product->price, locale: 'id') }}</p>
		 ...
		 <div>
			<h3 class="text-sm font-medium text-gray-900 mt-5">Stock : {{ $product->stock }}</h3>
		 </div>
		 ...
		 <form class="mt-10" action="{{ route('add_to_cart', $product) }}" method="post">
		   ...
		   <div class="mt-8">
			...
			<input type="number" name="amount" value="1" id="amount" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
		   </div>
		   @if($errors->any())
				<ul>
					@foreach($errors->all() as $error)
